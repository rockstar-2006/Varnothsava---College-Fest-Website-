...
async function run() {
    try {
        ...
        console.log("Calling /api/admin/export?type=registrations&eventId=TECH-001");
        const resMe = await fetch('http://localhost:3000/api/admin/export?type=registrations&eventId=TECH-001', {
            headers: {
                'Authorization': 'Bearer ' + token
            }
        });
        
        console.log("Export status:", resMe.status);
        console.log("Export body:", await resMe.text());
