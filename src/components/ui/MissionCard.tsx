                ...
                <div className={`px-8 pb-8 z-20 w-full mt-auto`}>
                    <div className="grid grid-cols-2 gap-3 w-full justify-center">
                        <button
                            ...
                            className={`relative py-4 md:py-3 text-xs font-black uppercase tracking-widest transition-all duration-300 overflow-hidden group/btn border-2 ${isRegClosedFlag && !isRegistered && isLoggedIn ? 'border-red-500 bg-red-500/20 text-red-500 cursor-not-allowed' : (event.type === 'Cultural' ? 'border-amber-500 bg-amber-500 text-black' : event.type === 'Business' ? 'border-sky-500 bg-sky-500 text-white' : 'border-emerald-500 bg-emerald-500 text-black')} shadow-[0_0_20px_rgba(0,0,0,0.4)] active:scale-95 flex items-center justify-center gap-2 touch-manipulation min-h-[48px]`}
                            disabled={isRegClosedFlag && !isRegistered && isLoggedIn}
                        >
                            {isRegistered && event.whatsappLink ? <MessageCircle className="w-4 h-4" /> : <UserPlus className="w-4 h-4" />}
                            {!isLoggedIn ? 'LOGIN' : (isRegClosedFlag && !isRegistered ? 'CLOSED' : (isRegistered ? (event.whatsappLink ? 'WHATSAPP' : 'REGISTERED') : (!hasPaid ? 'PAY FOR PASS' : 'REGISTER')))}
                        </button>
